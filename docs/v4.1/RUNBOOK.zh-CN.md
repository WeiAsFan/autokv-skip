...
v4.1 默认 `runtime.request_concurrency=4`，同一服务最多同时处理 4 个在途请求，以便 vLLM 合并批处理。该值同时用于 BF16、FP4、混合策略及所有阶段；需要串行对照可在新运行前改为 1。样本量、评分和选层规则不变，不承诺 4 倍加速，实际收益需服务器测量。

结果按完成顺序逐题落盘，后续统计按样本 ID 配对。请求预算包含在途请求和重试；耗尽或出错后停止新请求，已发出的请求会在返回或超时后收尾，成功结果仍保存。续跑只补缺失题。更换代码或并发配置应开始新运行，不能混用旧回答来比较性能。旧运行仍需原代码和配置续跑。
