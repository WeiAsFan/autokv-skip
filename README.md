...
AutoKV-Skip 是一个面向 AI 算法工程师面试的小型 vLLM 推理优化项目。当前 **v4.1** 自动选择部分层保留 BF16 KV，其余层使用 FP4 E2M1 KV；在独立测试集的质量约束内，争取相同 KV 显存预算下至少 `1.5×` 的实测容量。模型权重保持 BF16。

当前默认最多 4 个并发请求，让 vLLM 利用批处理；逐题保存与断点续跑保持可用，实际加速比需服务器测量。

本版保留“难度扫描 → 两批新数据确认 → 冻结规则 → 新实验集/测试集 → 束搜索与独立测试”，将 v4.0 的 FP8 实验迁移为 FP4。每批确认要求 BF16 正确率至少 `0.80`、BF16−P0 严格大于 `0.10`；最终总分容差仍为 `0.01`。计入每 16 元素一个 E4M3 scale 后，全 FP4 理论容量为 `32/9 ≈ 3.56×`。

使用 [v4.1 离线运行手册](docs/v4.1/RUNBOOK.zh-CN.md)。格式与适配边界见[设计说明](docs/v4.1/DESIGN.zh-CN.md)，旧版自动 scale 问题见[调查记录](docs/v4.1/SCALE-INVESTIGATION.zh-CN.md)。共同语言见 [CONTEXT.md](CONTEXT.md)。

A6000 使用 FP4 字节打包、FlashInfer FA2 软件解码和 BF16 attention，需要项目提供的隔离 vLLM 适配。当前完成了 CPU 数值与流程测试，**尚未在 A6000 上验证新内核编译、模型质量和实测容量**。PyTorch 写入实现以正确性为先，不能预先承诺吞吐加速。安装不会覆盖正在运行的 v4.0 环境。

服务器不能联网或推送；Linux 登录设备可联网但不能推送。文件离线传入服务器，产物传回 Linux 后在 GitHub 网页手动提交。不要求提交、推送、doctor 或 smoke 才能开始实验。历史文档和归档继续保留原来的 FP8 定义。
