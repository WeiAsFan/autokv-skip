...
AutoKV-Skip 是一个面向 AI 算法工程师面试的小型 vLLM 推理优化项目。它自动选择部分层保留 BF16 KV，其余层使用 FP8 E4M3 KV；在独立测试集的质量下降约束内，争取相同 KV 显存预算下至少 `1.5×` 的实测容量。不修改 vLLM/CUDA 内核，不量化模型权重。

当前分支为 **v4.0**，已实现“难度扫描与两批确认 → 冻结数据规则 → 新实验集/测试集 → 自动选层与独立测试”。使用 [v4.0 离线运行手册](docs/v4.0/RUNBOOK.zh-CN.md)的 `v4-run` 命令执行完整流程。目标定义见[共同语言](CONTEXT.md)，实现依据见[设计文档](docs/v4.0/DESIGN.zh-CN.md)与[实现计划](docs/v4.0/IMPLEMENTATION-PLAN.zh-CN.md)。

v4.0 默认要求两批新确认数据各自满足 BF16 正确率至少 `0.80`、BF16−P0 绝对分差严格大于 `0.10`；可在构造前将恢复需求改为 `0.20`。最终混合配置的总分质量容差仍为 `0.01`，实测容量目标仍为 `1.5×`。构造成功不代表已经找到可恢复的层集合。

v3.0 的 [GPU 结果已归档](results/v3-20260909T062152.212632Z/v3-a502366f75cc8179-QUALITY-v3.zh-CN.md)：实验集选择 P0，束搜索请求数为 0；独立测试质量未达标，实测容量达到 `2.0×`。因此 v4.0 先改进数据，复用原束搜索、删层和候选冻结算法。构造未找到时保留负结果；不会自动降低阈值或切到 FP4。

本版 CPU 验证覆盖数据、评分、两批确认、正式选层、断点续跑及离线导出，相关历史流程通过回归；还核对了真实 Mistral tokenizer 下的长度与证据布局，详见 [v4.0 验证记录](docs/v4.0/VALIDATION.zh-CN.md)。尚未运行 v4.0 GPU 实验，不能据此宣称构造或主目标已经达标。
