...
当前分支为 **v3.0**，已实现新数据、增量推理、束搜索、独立测试和结果导出。从 [v3.0 离线运行手册](docs/v3.0/RUNBOOK.zh-CN.md) 开始；方法见[设计文档](docs/v3.0/DESIGN.zh-CN.md)，交付顺序见[实现计划](docs/v3.0/IMPLEMENTATION-PLAN.zh-CN.md)，目标定义见[共同语言](CONTEXT.md)。当前完成代码与 CPU 开发验证，尚无 v3.0 正式 GPU 结果。

开发回归 170 项通过，范围与环境说明见[代码验证记录](docs/v3.0/VALIDATION.zh-CN.md)。
